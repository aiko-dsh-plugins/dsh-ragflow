...
const publicLabel = (value: string): string => value.replace(/ragflow|weknora/gi, 'Knowledge')
...
export function normalizeRetrieved(raw: unknown, selected: readonly Dataset[], limit: number): { results: { dataset: string; file: string; content: string; score?: number; citation?: { number: number; url: string } }[]; sources: DatasetSource[]; total: number } {
  ...
  for (const entry of raw.chunks) {
    if (!record(entry)) throw new Error('知识源返回了无效分块。')
    ...
    if (!datasetId || !fileId || !chunkId || !content.trim() || !datasets.has(datasetId)) {
      throw new Error('知识源分块缺少可核实的来源，或超出会话所选范围；已拒绝返回。')
    }
    if (results.length >= limit) continue
    const dataset = datasets.get(datasetId)!
    const datasetName = publicLabel(dataset.name)
    const metadata = record(entry.document_metadata) ? entry.document_metadata : {}
    const fileName = publicLabel(string(entry.document_name || metadata.name || entry.document_keyword) || fileId)
    let source = sources.get(datasetId)
    if (!source) { source = { provider: 'RAGFlow', id: datasetId, name: datasetName, files: [] }; sources.set(datasetId, source) }
    ...
    const result = { dataset: datasetName, file: fileName, content, ...(score === undefined ? {} : { score }) }
...
export function createScopedTool(port: McpPort, config: ResolvedConfig,
  ...
  return {
    ...
    async execute(args, exec) {
      if (!exec.agent) throw new Error('知识源检索需要可核实的会话，已拒绝无会话调用。')
